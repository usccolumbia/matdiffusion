...
class DiscreteDiffusionMatrixBase(DiscreteDiffusionBase):
    ...
    def sample_and_compute_posterior_q(self,
        ...
        # print("q_0", x_0.shape)
        # print('t', t.shape)
        # print('self prob_at_time_t', prob_at_time_t.shape)
        # print('&'*30)
        ...
        if return_logits:
            ...
            # skipped.
            # print('*'*50)
            # print('transition_probs', transition_probs.shape)
            # print('prob_at_time_t', prob_at_time_t.shape)
...
def p_forward(
    ...
    # print('%'*20 + ' first denoise_fn '+'%'*20)
    logits = denoise_fn(targets=x_t, timestep=t, attention_mask=target_mask)
    # print('x_t', x_t.shape)
    # print('t', t.shape)
    # print('target_mask', target_mask.shape)
    # # print('denoise_fn', denoise_fn)
    # print('logits', logits.shape) ### 30522
...
def compute_kl_reverse_process(x_start,
    ...
    # print('^'*20 + ' first p_forward '+ '^'*20)
    # print('transition_probs', transition_probs.shape)
    # # print('denoise_fn', denoise_fn)
    # print('target_mask', target_mask.shape)
    # print('x_t_plus_1', x_t_plus_1.shape)
    # print('t + step_size', (t+step_size).shape)
    # print('diffusion', diffusion)
    # # print('predict_x0',predict_x0.shape)
    # # print('log_space',log_space.shape)
    # print('transition_probs',transition_probs.shape)
    # print('word_freq_logits', word_freq_logits.shape)
    p_t = p_forward(
        denoise_fn,
        target_mask,
        x_t_plus_1,
...
def discrete_diffusion_predict_fn(
  ...
  """
    if show_process:
        tk = BertTokenizerFast.from_pretrained('./tokenizer/',  max_len = 130, do_lower_case=False)
    num_steps = diffusion.num_steps
    ...
    def sampling_step(step, state):
        ...
        # random_x = list(np.random.randint(71, size=(2,20)))
        # # print(random_x)
        # state.x = torch.tensor(random_x, device=device)
         # print('t: ', t)
        # print('x_t: ', state.x) ## mask?
        # exit()
        # print('^'*20 + 'second p_forward')
    ...
    if predict_x0:
        ...
        # init_state =torch.tensor([12,12,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4], device=device)
