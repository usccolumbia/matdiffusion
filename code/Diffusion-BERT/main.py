...
from models.modeling_roberta import RobertaForMaskedLM ### GPT
...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", default='bert-fast', type=str, required=False)
    parser.add_argument("--task_name", default='allmp', type=str, required=False)
...
if __name__ == '__main__':
    ...
    elif args.model_name_or_path in ['bert-fast']: #### our new
        model_cls = BertForMaskedLM ###### GPT
        cfg_cls = BertConfig
    ...
    tokenizer = BertTokenizerFast.from_pretrained('/home/rongzhid/Diffusion-BERT-mp/wo_DDP/tokenizer/', max_len = 130, do_lower_case=False)
    word_freq = torch.load(f'./word_freq/{args.model_name_or_path}_{args.task_name}.pt')
    # print('*^'*20)
    # print('word_freq.size(0)',word_freq.size(0))
    ...
    if args.from_scratch:
        print("load model 1")
        model = model_cls.from_pretrained(f"./seq_ckpts/best(369).th", config=cfg).to(device)
    ...
    train_data, test_data = FormulaLoader(tokenizer=tokenizer).my_load(task_name ='mp')
    # train_data, test_data = train_data['seq'], test_data['seq']
    # print(len(test_data['input_ids']))
    train_data, dev_data = train_data.train_test_split(train_size = 0.9).values()
    # train_data, dev_data = train_data.train_test_split(test_size=0.1)
    # print(len(dev_data['input_ids']))
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, collate_fn=collate_fn, num_workers=1, pin_memory=True, sampler = train_sampler)
    dev_loader = torch.utils.data.DataLoader(dev_data, batch_size=args.batch_size, collate_fn=collate_fn, num_workers=1, pin_memory=True, sampler=dev_sampler)
    ...
    if args.timestep == 'none':
        def denoise_fn(targets, timestep, attention_mask):
            ...
            print('*'*20, targets.shape, attention_mask.shape)
