    ...
    *, data_dir, batch_size, image_size, class_cond=False, deterministic=False, data_args=None, 
    ...
    if task_mode == 'roc' or task_mode == 'roc-aug' :
        ...
        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                            padding_mode=padding_mode, split=split,
                                            load_vocab=load_vocab)
    elif task_mode == 'seq2seq':
        print('hello loading formula sequence ')
        tokenizer = BertTokenizerFast.from_pretrained('/home/rongzhid/Diffusion-LM-mp/improved-diffusion/scripts/tokenizer/', max_len = 130, do_lower_case=False)
        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                            padding_mode=padding_mode, split=split,
                                            load_vocab=load_vocab)
...
def helper_tokenize_encode(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode, ):
    ...
    with torch.no_grad():
        for input_ids in sentence_lst:
            # tokenized_ = [vocab_dict.get(x, vocab_dict['UNK']) for x in input_ids]
            # input_ids = [0] + tokenized_ + [1]
            tokenizer = BertTokenizerFast.from_pretrained('/home/rongzhid/Diffusion-LM-mp/improved-diffusion/scripts/tokenizer/')
            input_ids = tokenizer.encode("Al O .")
            group_lst['word_ids'].append(input_ids)
        print(group_lst['word_ids'][:2])
...
def get_corpus_rocstory(data_args, model, image_size, padding_mode='block',
    ...
    if data_args.experiment_mode == 'lm':
        ...
        elif data_args.modality == 'seq2seq':
            print('loading dataset from simple mp dataset')
            sentence_lst = []
            nlp = English()
            tokenizer = nlp.tokenizer
            if split == 'train':
                print('loading form the TRAIN set')
                path = f'{data_args.seq_train}/src1_train.txt'
            elif split == 'valid':
                print('loading form the VALID set')
                path = f'{data_args.seq_train}/src1_valid.txt'
            elif split == 'test':
                print('loading form the TEST set')
                path = f'{data_args.seq_train}/src1_test.txt'
            elif split == 'debug':
                print('loading form the DEBUG set')
                path = data_args.debug_path
                import json
    ...
    if load_vocab is None:
        ...
        if not os.path.exists(data_args.checkpoint_path):
            os.mkdir(data_args.checkpoint_path)
        path_save_vocab = f'{data_args.checkpoint_path}/vocab.json'
        print(f'1 save the new vocab to {path_save_vocab}')
        ...
        if not os.path.exists(path_save_vocab):
            print(f'2 save the vocab to {path_save_vocab}')
            # if isinstance(vocab_dict, dict):
            with open(path_save_vocab, 'w') as f:
                json.dump(vocab_dict, f)
